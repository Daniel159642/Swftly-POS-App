...
def get_image_matcher():
    ...
    if _image_matcher is None:
        try:
            from src.ai.product_image_matcher import ProductImageMatcher
...
def _send_order_notification_async(order_info):
    ...
    def _do():
        try:
            from src.services.notification_service import send_order_notification, get_order_email_recipients, get_order_sms_recipients
...
def api_inventory():
    ...
    if request.method == 'POST':
        ...
        try:
            from src.core.database import ensure_metadata_tables
...
def api_update_inventory(product_id):
    ...
    try:
        ...
        if new_quantity is not None:
            try:
                from src.core.database import get_connection as _gc
        ...
        from src.core.database import update_product
        ...
        if success and new_quantity is not None and old_quantity is not None:
            try:
                ...
                if abs(qty_diff) >= 0.01 and _inv_cost > 0:
                    ...
                    from src.integrations.pos_accounting_bridge import journalize_inventory_adjustment_to_accounting
...
def api_delete_inventory(product_id):
    ...
    try:
        from src.core.database import delete_product
...
def api_archive_inventory(product_id):
    ...
    try:
        from src.core.database import set_archived_product
...
def api_product_variants(product_id):
    ...
    from src.core.database import get_product, get_product_variants, add_product_variant
...
def api_product_variant_by_id(variant_id):
    ...
    from src.core.database import update_product_variant, delete_product_variant, get_variant_by_id
...
def api_product_ingredients(product_id):
    ...
    from src.core.database import get_product_ingredients, add_product_ingredient
    if request.method == 'GET':
        try:
...
def api_delete_product_ingredient(recipe_id):
    ...
    from src.core.database import delete_product_ingredient
    try:
...
def api_vendor_by_id(vendor_id):
    if request.method == 'DELETE':
        try:
            from src.core.database import delete_vendor
...
def api_archive_vendor(vendor_id):
    ...
    try:
        from src.core.database import set_archived_vendor
...
def api_update_vendor_impl(vendor_id):
    """Update a vendor"""
    try:
        from src.core.database import update_vendor
...
def api_category_by_id(category_id):
    if request.method == 'DELETE':
        try:
            from src.core.database import delete_category
...
def api_archive_category(category_id):
    ...
    try:
        from src.core.database import set_archived_category
...
def api_update_category_impl(category_id):
    ...
    try:
        from src.core.database import get_connection, create_or_get_category_with_hierarchy
        data = request.json if request.is_json else request.form.to_dict()
    ...
    try:
        ...
        if 'photo' in request.files:
            ...
            if photo.filename:
                ...
                try:
                    from src.core.database import get_product, update_product
                    conn, cursor = _pg_conn()
                    try:
...
def api_save_shipment_draft():
    ...
    try:
        ...
        from src.core.database import save_shipment_draft
...
def api_load_draft(draft_id):
    ...
    try:
        from src.core.database import get_pending_shipment_details
    ...
    try:
        ...
        if items_data:
            ...
            if vendor_id:
                ...
                from src.core.database import get_pending_shipment
...
def api_login():
    ...
    try:
        ...
        try:
            from src.core.database import check_login_rate_limit, record_login_attempt
...
def api_admin_login():
    ...
    try:
        ...
        from src.core.database import manager_login
...
def api_setup_update_establishment():
    ...
    try:
        ...
        try:
            ...
            if store_type:
                try:
                    from src.integrations.accounting_bootstrap import set_establishment_vertical
...
def api_setup_create_admin():
    ...
    try:
        ...
        from src.core.database import create_admin_account
...
def api_webauthn_register_begin():
    ...
    try:
        ...
        if not employee:
            return jsonify({'success': False, 'message': 'Authentication required'}), 401
        from src.core.database import webauthn_register_begin
...
def api_webauthn_register_complete():
    ...
    try:
        ...
        from src.core.database import webauthn_register_complete
...
def api_webauthn_login_begin():
    ...
    try:
        ...
        from src.core.database import webauthn_login_begin
...
def api_webauthn_login_complete():
    ...
    try:
        ...
        from src.core.database import webauthn_login_complete, record_login_attempt
...
def api_webauthn_list_mine():
    ...
    try:
        ...
        if not employee:
            return jsonify({'success': False, 'message': 'Authentication required'}), 401
        from src.core.database import list_webauthn_credentials
...
def api_webauthn_delete_credential(cred_id):
    ...
    try:
        ...
        from src.core.database import delete_webauthn_credential
...
def api_webauthn_list_all():
    ...
    try:
        ...
        from src.core.database import list_all_webauthn_credentials
...
def api_create_order():
    ...
    try:
        ...
        data = request.json
        from src.core.database import create_order
...
def api_generate_transaction_receipt(transaction_id):
    ...
    try:
        from src.services.receipt_generator import generate_transaction_receipt
...
def api_generate_test_receipt():
    ...
    try:
        from src.services.receipt_generator import generate_test_receipt_pdf
...
def api_generate_receipt(order_id):
    ...
    try:
        from src.services.receipt_generator import generate_receipt_with_barcode
...
def api_pos_bootstrap():
    ...
    try:
        ...
        try:
            ...
            from src.core.database import ensure_metadata_tables, list_categories
            ensure_metadata_tables()
            sql = """
...
def api_update_pos_search_filters():
    ...
    try:
        ...
        from src.core.database import update_establishment_settings
...
def api_void_order(order_id):
    ...
    try:
        ...
        if not employee_id:
            return jsonify({'success': False, 'message': 'Employee ID or session required'}), 401
        from src.core.database import void_order
...
def api_update_order_status(order_id):
    ...
    try:
        ...
        if not employee_id:
            employee_id = data.get('employee_id')
        from src.core.database import update_order_status, get_order_details
            ...
            if new_status == 'ready':
                try:
                    ...
                    if o and (o.get('order_source') or '').strip().lower() == 'doordash' and (o.get('external_order_id') or '').strip():
                        from src.core.database import get_integrations
                        ...
                        if establishment_id is not None:
                            ...
                            if dd and isinstance(dd.get('config'), dict):
                                from src.integrations.doordash_service import notify_doordash_order_ready
                ...
                try:
                    ...
                    if o and (o.get('order_source') or '').strip().lower() == 'doordash' and (o.get('external_order_id') or '').strip():
                        ...
                        if establishment_id is not None:
                            ...
                            dd = next((i for i in (integrations or []) if i.get('provider') == 'doordash' and i.get('enabled')), None)
                            if dd and isinstance(dd.get('config'), dict):
                                from src.integrations.doordash_service import cancel_doordash_order
...
def api_order_doordash_lines(order_id):
    ...
    try:
        from src.core.database import get_order_details, get_doordash_order_lines
        o = get_order_details(order_id)
...
def api_order_doordash_adjustment(order_id):
    ...
    try:
        ...
        from src.core.database import get_order_details, get_integrations
        o = get_order_details(order_id)
        if not o or (o.get('order_source') or '').strip().lower() != 'doordash':
            return jsonify({'success': False, 'message': 'Order not found or not a DoorDash order'}), 404
        ...
        from src.integrations.doordash_service import adjust_doordash_order
...
def api_order_doordash_order_manager_url(order_id):
    ...
    try:
        ...
        if not dd or not isinstance(dd.get('config'), dict):
            return jsonify({'success': False, 'message': 'DoorDash integration not enabled for this store'}), 400
        from src.integrations.doordash_service import build_live_order_manager_url
...
def api_shopify_callback():
    ...
    try:
        ...
        from src.core.database import upsert_integration
...
def api_save_integrations():
    ...
    try:
        ...
        if not isinstance(config, dict):
            config = {}
        from src.core.database import upsert_integration, create_or_get_category_with_hierarchy
...
def api_migrations_square_status():
    ...
    try:
        from src.core.database import get_establishment_settings
        establishment_id = get_current_establishment() if get_current_establishment else None
...
def api_migrations_square_oauth_callback():
    ...
    try:
        ...
        redirect_uri = _square_redirect_uri()
        sandbox = False
        from src.integrations.square_migration import exchange_code_for_tokens, verify_connection
        from src.core.database import update_establishment_settings
...
def api_migrations_square_verify():
    ...
    try:
        ...
        if not access_token:
            establishment_id = get_current_establishment() if get_current_establishment else None
            from src.integrations.square_migration import get_valid_square_access_token
            access_token, err = get_valid_square_access_token(establishment_id, sandbox=sandbox)
            if err:
                return jsonify({'success': False, 'message': err}), 400
        from src.integrations.square_migration import verify_connection
...
def api_migrations_square_run():
    ...
    try:
        ...
        if get_current_establishment is not None:
            establishment_id = get_current_establishment()
        from src.integrations.square_migration import run_migration
...
def api_migrations_square_upload():
    ...
    try:
        ...
        from src.integrations.square_import import parse_square_items_file, run_square_file_import
...
def api_orders_from_integration():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_integrations, create_order, get_connection
        from psycopg2.extras import RealDictCursor
        integrations = get_integrations(establishment_id)
...
def api_shopify_sync():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.integrations.shopify_service import sync_shopify_orders
...
def api_shopify_sync_products():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.integrations.shopify_service import sync_shopify_products
...
def api_webhooks_shopify_orders():
    ...
    try:
        ...
        from src.core.database import get_connection, get_integrations
        from psycopg2.extras import RealDictCursor
        conn = get_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        ...
        if not establishment_id:
            return jsonify({'ok': False}), 400
        from src.integrations.shopify_service import build_pos_payload_from_shopify_order
        from src.core.database import create_order, get_connection
        from psycopg2.extras import RealDictCursor
        integrations = get_integrations(establishment_id)
...
def api_webhooks_doordash_orders():
    ...
    try:
        ...
        if event and (event.get("type") or "").strip().upper() != "ORDERCREATE":
            return jsonify({'ok': True}), 200
        from src.core.database import get_connection, get_integrations, create_order
        from psycopg2.extras import RealDictCursor
        conn = get_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        ...
        if not isinstance(config, dict):
            config = {}
        price_multiplier = float(config.get('price_multiplier') or 1)
        from src.integrations.doordash_service import build_pos_payload_from_doordash_order, confirm_doordash_order
        ...
        from src.integrations.doordash_service import FAILURE_REASON_CONNECTIVITY
        if not employee_id:
        ...
        if result.get('success') and result.get('order_id'):
            try:
                from src.core.database import save_doordash_order_lines
...
def api_webhooks_doordash_order_release():
    ...
    try:
        ...
        if not client_order_id:
            return jsonify({'ok': True}), 200
        from src.core.database import get_connection, update_order_status
        from psycopg2.extras import RealDictCursor
        conn = get_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
...
def api_webhooks_doordash_order_adjustment():
    ...
    try:
        ...
        if not external_id:
            return jsonify({'ok': True}), 200
        from src.core.database import get_order_by_external_order_id, get_connection, replace_doordash_order_lines, update_order_totals_from_doordash
        from psycopg2.extras import RealDictCursor
        ...
        if not isinstance(config, dict):
            config = {}
        price_multiplier = float(config.get('price_multiplier') or 1)
        from src.integrations.doordash_service import _flatten_order_items
    ...
    try:
        ...
        if dasher_obj and isinstance(dasher_obj, dict):
            ...
            if not any(dasher_info.values()):
                dasher_info = None
        from src.core.database import update_order_dasher_status
    ...
    try:
        ...
        if merchant_supplied_id:
            try:
                ...
                pass
        if establishment_id is not None:
            from src.core.database import insert_doordash_store_deactivation_event
...
def api_webhooks_doordash_order_cancellation():
    ...
    try:
        ...
        if not external_order_id and not client_order_id:
            return jsonify({'ok': True}), 200
        from src.core.database import get_order_by_external_order_id, update_order_status
        pos_order = None
        ...
        if not pos_order and client_order_id:
            try:
                ...
                from src.core.database import get_order_details
...
def api_doordash_menu_pull(location_id):
    ...
    try:
        ...
        from src.integrations.doordash_service import build_doordash_menu_pull_response
    ...
    try:
        ...
        if menu_id and merchant_supplied_id:
            try:
                ...
                establishment_id = None
            if establishment_id is not None:
                from src.core.database import set_doordash_menu_uuid
...
def api_doordash_push_menu():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_integrations
        from src.integrations.doordash_service import push_doordash_menu
        integrations = get_integrations(establishment_id)
        integration = next((i for i in integrations if i.get('provider') == 'doordash' and i.get('enabled')), None)
        if not integration:
            return jsonify({'success': False, 'message': 'DoorDash integration not enabled'}), 400
...
def api_doordash_push_store_hours():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_integrations, update_store_location_settings
        from src.integrations.doordash_service import push_doordash_menu
        integrations = get_integrations(establishment_id)
        integration = next((i for i in integrations if i.get('provider') == 'doordash' and i.get('enabled')), None)
        if not integration:
...
def api_doordash_add_all_items():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_integrations, get_connection
        integrations = get_integrations(establishment_id)
        integration = next((i for i in integrations if i.get('provider') == 'doordash' and i.get('enabled')), None)
        if not integration:
            return jsonify({'success': False, 'message': 'DoorDash integration not enabled'}), 400
        from src.core.database import create_or_get_category_with_hierarchy, assign_category_to_product
        conn = get_connection()
        cur = conn.cursor()
        try:
            cur.execute("SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'inventory' AND column_name = 'sell_at_pos'")
...
def api_doordash_store_menu():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_integrations
        from src.integrations.doordash_service import get_doordash_store_menu
        integrations = get_integrations(establishment_id)
        integration = next((i for i in integrations if i.get('provider') == 'doordash' and i.get('enabled')), None)
        if not integration:
            return jsonify({'success': False, 'message': 'DoorDash integration not enabled'}), 400
...
def api_doordash_store_menu_flat():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_integrations
        from src.integrations.doordash_service import get_doordash_store_menu, flatten_doordash_store_menu_for_sync
        integrations = get_integrations(establishment_id)
        integration = next((i for i in integrations if i.get('provider') == 'doordash' and i.get('enabled')), None)
        if not integration:
            return jsonify({'success': False, 'message': 'DoorDash integration not enabled'}), 400
...
def api_doordash_menu_sync():
    ...
    try:
        ...
        from src.core.database import get_integrations, upsert_integration
        integrations = get_integrations(establishment_id)
...
def api_doordash_store_details():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_integrations
        from src.integrations.doordash_service import get_doordash_store_details
        integrations = get_integrations(establishment_id)
        integration = next((i for i in integrations if i.get('provider') == 'doordash' and i.get('enabled')), None)
        if not integration:
            return jsonify({'success': False, 'message': 'DoorDash integration not enabled'}), 400
...
def api_doordash_menu_details():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_integrations
        from src.integrations.doordash_service import get_doordash_menu_details
        integrations = get_integrations(establishment_id)
        integration = next((i for i in integrations if i.get('provider') == 'doordash' and i.get('enabled')), None)
        if not integration:
            return jsonify({'success': False, 'message': 'DoorDash integration not enabled'}), 400
...
def api_doordash_store_status():
    ...
    try:
        ...
        from src.core.database import get_integrations
        from src.integrations.doordash_service import update_doordash_store_status
        integrations = get_integrations(establishment_id)
        integration = next((i for i in integrations if i.get('provider') == 'doordash' and i.get('enabled')), None)
        if not integration:
            return jsonify({'success': False, 'message': 'DoorDash integration not enabled'}), 400
...
def api_doordash_latest_store_deactivation():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({}), 200
        from src.core.database import get_latest_doordash_store_deactivation
...
def api_create_demo_integration_orders():
    ...
    try:
        ...
        if not establishment_id:
            return jsonify({'success': False, 'message': 'No establishment'}), 400
        from src.core.database import get_connection, create_order
        from psycopg2.extras import RealDictCursor
...
def api_orders_latest():
    ...
    try:
        from src.core.database import list_orders
...
def api_employee_schedule():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            data = request.json
            from src.core.database import add_schedule, add_calendar_event, get_employee
            ...
            if session_token:
                try:
                    from src.core.database import verify_session
                    session_data = verify_session(session_token)
...
def api_clock_in():
    ...
    try:
        ...
        if clock_result['success']:
            ...
            try:
                from src.services.notification_service import send_clockin_notification
                import threading
...
def api_get_clockin_notification_settings():
    ...
    try:
        ...
        from src.services.notification_service import get_clockin_notification_settings
...
def api_save_clockin_notification_settings():
    ...
    try:
        ...
        from src.services.notification_service import save_clockin_notification_settings
...
def api_get_schedule_notification_settings():
    ...
    try:
        store_id = int(request.args.get('store_id', 1))
        from src.services.notification_service import get_schedule_notification_settings
...
def api_save_schedule_notification_settings():
    ...
    try:
        data = request.json or {}
        store_id = int(data.get('store_id', 1))
        from src.services.notification_service import save_schedule_notification_settings
...
def api_get_register_notification_settings():
    ...
    try:
        store_id = int(request.args.get('store_id', 1))
        from src.services.notification_service import get_register_notification_settings
...
def api_save_register_notification_settings():
    ...
    try:
        data = request.json or {}
        store_id = int(data.get('store_id', 1))
        from src.services.notification_service import save_register_notification_settings
        ids = data.get('admin_email_ids', [])
        if isinstance(ids, list):
            ids = [int(i) for i in ids if str(i).strip().isdigit()]
        else:
...
def api_dashboard_statistics():
    ...
    try:
        ...
        try:
            ...
            from src.integrations.accounting_bootstrap import ensure_accounting_schema, ensure_establishment_accounting
            ensure_accounting_schema()
            if stats_establishment_id:
                ensure_establishment_accounting(stats_establishment_id)
            from src.core.database import get_income_summary, get_income_buckets
...
def api_identify_product():
    ...
    try:
        ...
        try:
            from src.services.barcode_scanner import smart_product_identification
...
def api_create_return():
    ...
    try:
        ...
        data = request.json
        from src.core.database import process_return_immediate
...
def api_process_return():
    ...
    try:
        ...
        data = request.json
        from src.core.database import process_return_immediate
        from src.services.receipt_generator import generate_return_receipt, generate_exchange_receipt
        ...
        if employee_id and result.get('return_total'):
            try:
                from src.integrations.pos_accounting_bridge import journalize_return_to_accounting
...
def api_generate_return_receipt(return_id):
    ...
    try:
        from src.services.receipt_generator import generate_return_receipt
...
def api_generate_exchange_receipt(exchange_credit_id):
    ...
    try:
        from src.services.receipt_generator import generate_exchange_receipt
        from src.core.database import get_connection
        from psycopg2.extras import RealDictCursor
...
def api_generate_exchange_completion_receipt(order_id):
    ...
    try:
        from src.services.receipt_generator import generate_exchange_completion_receipt
        from src.core.database import get_connection
        from psycopg2.extras import RealDictCursor
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                from src.integrations.pos_accounting_bridge import journalize_exchange_credit_applied_to_accounting
...
def api_generate_schedule():
    ...
    try:
        from src.services.schedule_generator import AutomatedScheduleGenerator
...
def _trigger_schedule_update_email(period_id: int, changed_emp_ids: list = None):
    ...
    try:
        from src.services.notification_service import send_schedule_notification_advanced, send_schedule_notification
        import threading
        from datetime import datetime, timedelta
...
def api_master_calendar():
    ...
    try:
        from src.core.database import get_calendar_events
...
def api_create_calendar_event():
    ...
    try:
        ...
        from src.core.database import add_calendar_event
...
def calendar_feed(token):
    ...
    try:
        from src.integrations.calendar_integration import CalendarIntegrationSystem
...
def api_google_calendar_connect_url():
    ...
    try:
        ...
        if not session_data.get('valid'):
            return jsonify({'success': False, 'message': 'Invalid session'}), 401
        employee_id = session_data['employee_id']
        from src.integrations.google_calendar_sync import get_oauth_url
...
def api_google_calendar_callback():
    ...
    try:
        ...
        from src.integrations.google_calendar_sync import exchange_code_for_tokens, save_tokens
...
def api_google_calendar_status():
    ...
    try:
        ...
        if not session_data.get('valid'):
            return jsonify({'success': True, 'connected': False})
        from src.integrations.google_calendar_sync import get_stored_tokens
...
def api_quickbooks_connect_url():
    ...
    try:
        ...
        from src.integrations.quickbooks_sync import get_oauth_url
        url = get_oauth_url()
        return jsonify({'success': True, 'url': url})
    except ValueError as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
def api_quickbooks_callback():
    ...
    try:
        ...
        from src.integrations.quickbooks_sync import exchange_code_for_tokens
...
def api_quickbooks_status():
    ...
    try:
        ...
        from src.integrations.quickbooks_sync import get_stored_tokens
...
def api_quickbooks_sync_accounts():
    ...
    try:
        ...
        from src.integrations.quickbooks_sync import sync_qbo_accounts_to_db
...
def api_quickbooks_sync_customers():
    try:
        ...
        from src.integrations.quickbooks_sync import sync_qbo_customers_to_db
...
def api_quickbooks_sync_vendors():
    try:
        ...
        session_data = verify_session(session_token)
        if not session_data.get('valid'): return jsonify({'success': False, 'message': 'Invalid session'}), 401
        from src.integrations.quickbooks_sync import sync_qbo_vendors_to_db
...
def api_quickbooks_sync_inventory():
    try:
        ...
        session_data = verify_session(session_token)
        if not session_data.get('valid'): return jsonify({'success': False, 'message': 'Invalid session'}), 401
        from src.integrations.quickbooks_sync import sync_qbo_products_to_db
...
def api_quickbooks_sync_transactions():
    try:
        ...
        session_data = verify_session(session_token)
        if not session_data.get('valid'): return jsonify({'success': False, 'message': 'Invalid session'}), 401
        from src.integrations.quickbooks_sync import sync_transactions_to_qbo
...
def api_google_calendar_sync():
    ...
    try:
        ...
        event = dict(row)
        from src.integrations.google_calendar_sync import sync_event_to_google, update_master_calendar_google_event_id
...
def start_transaction():
    ...
    try:
        ...
        discount_type = (data.get('discount_type') or '').strip() or None
        scheduled_time = data.get('scheduled_time')
        from src.services.customer_display_system import CustomerDisplaySystem
        ...
        if data.get('receipt_type') == 'email' and (data.get('email') or data.get('to_address')):
            ...
            if to_address:
                try:
                    ...
                    if order_id:
                        from src.services.notification_service import send_receipt_email_for_order
...
def api_verification_settings():
    ...
    try:
        from src.core.database import ensure_shipment_verification_tables
...
def api_add_to_inventory(shipment_id):
    ...
    try:
        ...
        if result.get('success'):
            ...
            try:
                from src.integrations.pos_accounting_bridge import journalize_shipment_received_to_accounting
...
def api_validate_stripe_credentials():
    ...
    try:
        ...
        try:
            ...
            from src.core.encryption_utils import encrypt
...
def api_stripe_terminal_connection_token():
    ...
    """
    try:
        from src.integrations.terminal_adapters import StripeTerminalAdapter
        data = request.json or {}
...
def api_create_payment_intent():
    ...
    try:
        ...
        from src.integrations.terminal_adapters import get_adapter
...
def api_notification_settings_get():
    ...
    try:
        store_id = int(request.args.get('store_id', 1))
        from src.services.notification_service import _get_sms_settings, get_notification_preferences
...
def api_email_templates_send_test():
    ...
    try:
        ...
        store_id = int(data.get('store_id', 1))
        from src.services.notification_service import send_email, _get_email_logo_png_bytes, _EMAIL_LOGO_CID, _EMAIL_LOGO_FILES
...
def api_email_templates_reset_to_la_maison():
    ...
    if not ok:
        return err
    try:
        from src.services.la_maison_templates import LA_MAISON_DEFAULTS
        data = request.json or {}
        store_id = int(data.get('store_id', 1))
        conn, cursor = _pg_conn()
...
def api_email_template_preview():
    ...
    if not ok:
        return err
    try:
        from src.services.notification_service import render_template
        data = request.json or {}
        subject = (data.get('subject_template') or '').strip()
        body_html = (data.get('body_html_template') or '').strip()
        body_text = (data.get('body_text_template') or '').strip()
...
def api_open_register():
    ...
    try:
        ...
        from src.core.database import open_cash_register
        ...
        if result.get('success'):
            try:
                from src.services.notification_service import send_register_notification
...
def api_close_register():
    ...
    try:
        ...
        from src.core.database import close_cash_register
        ...
        if result.get('success'):
            ...
            if result.get('discrepancy') is not None and abs(float(result.get('discrepancy', 0))) >= 0.01:
                try:
                    from src.integrations.pos_accounting_bridge import journalize_register_close_to_accounting
...
def api_add_cash_transaction():
    ...
    try:
        ...
        from src.core.database import add_cash_transaction
        ...
        if result.get('success'):
            ...
            try:
                from src.integrations.pos_accounting_bridge import journalize_cash_transaction_to_accounting
...
def api_get_register_session():
    ...
    try:
        ...
        from src.core.database import get_register_session
...
def api_get_register_summary():
    ...
    try:
        ...
        from src.core.database import get_register_summary
...
def api_reconcile_register():
    ...
    try:
        ...
        from src.core.database import reconcile_register_session
...
def api_get_register_events():
    ...
    try:
        ...
        from src.core.database import get_register_events
...
def api_register_cash_settings():
    ...
    try:
        ...
        from src.core.database import get_register_cash_settings, save_register_cash_settings
...
def api_daily_cash_count():
    ...
    try:
        ...
        from src.core.database import get_daily_cash_counts, save_daily_cash_count
        ...
        if request.method == 'GET':
            ...
            if result.get('success'):
                ...
                if count_type == 'drop' and float(total_amount or 0) > 0 and result.get('count_id'):
                    try:
                        from src.integrations.pos_accounting_bridge import journalize_cash_drop_to_accounting
...
if _ACCOUNTING_BACKEND_AVAILABLE:
    ...
    def api_accounting_reports_save():
        ...
        try:
            ...
            if save_as_pdf:
                try:
                    from src.services.report_pdf_generator import generate_report_pdf
...
def api_gift_card_sale():
    ...
    try:
        ...
        from src.integrations.pos_accounting_bridge import journalize_gift_card_sale_to_accounting
...
def api_gift_card_breakage(gift_card_id):
    ...
    try:
        ...
        if amount <= 0:
            return jsonify({'success': False, 'message': 'Amount must be positive'}), 400
        from src.integrations.pos_accounting_bridge import journalize_gift_card_breakage_to_accounting
...
def api_accounting_reconciliation():
    ...
    try:
        ...
        from src.core.database import get_reconciliation_report
...
def api_establishment_vertical(establishment_id):
    ...
    try:
        ...
        if request.method == 'GET':
            from src.integrations.accounting_bootstrap import get_accounting_settings
...
if __name__ == '__main__':
    ...
    try:
        from src.services.notification_service import scheduled_orders_worker
